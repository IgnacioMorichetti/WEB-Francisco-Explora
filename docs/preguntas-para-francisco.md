# Preguntas para Francisco

Contestá lo que puedas debajo de cada pregunta. No hace falta que estén todas. Lo que no sepas o no quieras, dejalo en blanco.

---

## Sobre vos

1. En una o dos frases, ¿cómo te presentarías? (lo que iría arriba de todo en la web)
   →

2. Contame tu historia en unos párrafos: de dónde sos, a qué te dedicás, cómo llegaste a esto.
   →

3. ¿Tenés alguna frase o lema que te represente?
   →

4. ¿Qué aventuras, viajes o logros tuyos te gustaría mostrar?
   →

---

## El Desafío (Medio Ironman)

5. Link del grupo de WhatsApp para el botón "Sumate al desafío":
   →

6. El botón "Sos una marca" (para que las marcas se sumen), ¿a dónde lleva? (¿tu WhatsApp? ¿un email?). Pasame el link
   →

7. ¿Dónde es la carrera y cómo se llama el evento?, si es que le vas a poner un nombre. Es el 28 de Junio ¿verdad?
   →

8. ¿Por qué este desafío? ¿Qué significa para vos?
   →

9. ¿Tenés un objetivo de tiempo o una meta personal?
   →
---

## Fotos

11. ¿Qué fotos tenés para usar y qué te gustaría que muestren? (aventuras, entrenamientos, viajes)
    →

12. ¿Las tenés en buena calidad? ¿Son tuyas o hay que dar crédito a alguien (Nombrarlos por canjes o cosas así)?
    →

13. ¿Tenés videos que quieras incluir?
    →

---

## Redes y contacto

14. Pasame tus redes (las que quieras mostrar): Instagram, TikTok, YouTube, Strava, Facebook, etc.
    →

15. ¿Cuál red querés hacer crecer más?
    →

16. ¿Qué email querés usar para que te contacten las marcas?
    →

---

## Marcas

17. ¿Hay alguna marca que ya te acompañe?
    →

---

## Gustos para la web

19. Ya me dijiste que la querés con onda natural / selvática (verdes oscuros y marrones). ¿Hay algún color, foto o web que te guste de referencia?
    →

20. ¿Tenés un logo o algún símbolo que te identifique?
    →

21. ¿Cómo querés que suene el texto de la web? (cercano, épico, divertido, inspirador...)
    →

22. ¿Hay algo que NO quieras que aparezca en la web? (Muy determinante)
    →
